...
class RuleContext
{
  ...
  private boolean termMatch(Term term, Term pattern)
  {
    switch (pattern.getValueType())
    {
      ...
      case CONSTANT:
        terms.add(term);
        final Constant p = pattern.asConstant();
  ...
  private boolean termMatch(CompoundTerm term, CompoundTerm pattern)
  {
    return term.getConnector() == pattern.getConnector()
        ...
        && term.getPlaceHolderPosition() == pattern.getPlaceHolderPosition()
        && termMatch(term.getTerms(), pattern.getTerms());
  }

  private boolean termMatch(List<Term> terms, List<Term> patterns)
  {
    boolean result = true;
    for (int i = 0; i < terms.size() && result; i++)
    {
      result = termMatch(terms.get(i), patterns.get(i));
    }
    return result;
  }

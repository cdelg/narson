...
final public class NarseseGeneratorImpl implements NarseseGenerator, NarseseChars, NarseseOperations
{
  ...
  public NarseseGeneratorImpl(final Writer writer, final int bufferSize, int prefixThreshold)
  {
  ...
  public NarseseGeneratorImpl(final OutputStream out, int bufferSize, int prefixThreshold)
  {
    this(new OutputStreamWriter(out, UTF8SET), bufferSize, prefixThreshold);
  }

  public NarseseGeneratorImpl(final OutputStream out, final Charset encoding, final int bufferSize,
      int prefixThreshold)
  {
      ...
      throws NarseseException, NarseseGenerationException
  {
    ...
    switch (value.getValueType())
    {
      case COMPOUND_TERM:
        ...
        {
          final List<Term> terms = new ArrayList<>(c.getTerms());
        ...
        break;
      case IMAGE_TERM:
        final ImageTerm image = (ImageTerm) value;
        writeStartCompoundTerm();
        switch (image.getConnector())
        {
        ...
        final List<Term> terms = image.getTerms();
        for (int i = 0; i < image.getPlaceHolderPosition() - 1; i++)
        {
          write(terms.get(i));
        }
        writePlaceHolder();
        for (int i = image.getPlaceHolderPosition() - 1; i < terms.size(); i++)
        {
          write(terms.get(i));
        }
        writeEnd();
        break;
      case SET_TERM:
        final SetTerm set = (SetTerm) value;
        writeStartCompoundTerm();
        switch (set.getConnector())
        {
        ...
        set.getTerms().forEach(this::write);
        writeEnd();
        break;
      case NEGATION_TERM:
        writeStartCompoundTerm();
        writeNegation();
        write(((NegationTerm) value).getTerm());
        writeEnd();
        break;
      case CONSTANT_TERM:
        writeConstant(((ConstantTerm) value).getName());
        break;
      ...
      case JUDGMENT:
        ...
        break;
      case OPERATION_TERM:
        final OperationTerm o = (OperationTerm) value;
        writeStartCompoundTerm();
      ...
      throws NarseseException, NarseseGenerationException
  {
    ...
    checkGeneratorState(WordHelper.isWordValid(name), () -> "Invalid name: " + name);
    prepareTerm();
  ...
  {
    checkGeneratorState(name != null, "Invalid null name.");
    checkGeneratorState(name.isEmpty() || WordHelper.isWordValid(name),
        () -> "Invalid name: " + name);
    prepareTerm();

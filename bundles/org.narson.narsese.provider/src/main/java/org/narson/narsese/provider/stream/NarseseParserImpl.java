...
import org.narson.narsese.provider.stream.ParseResult.SpecialEvent;

final public class NarseseParserImpl implements NarseseParser, NarseseChars
{
  ...
  public NarseseParserImpl(final Reader reader, int bufferSize)
  {
    this.reader = new BufferedReader(reader, bufferSize);
  }

  public NarseseParserImpl(final InputStream in, int bufferSize)
  {
    this(new InputStreamReader(in, UTF8_CHARSET), bufferSize);
  }

  public NarseseParserImpl(final InputStream in, final Charset encoding, int bufferSize)
  {
